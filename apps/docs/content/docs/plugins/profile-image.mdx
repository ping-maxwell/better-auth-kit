    ...
    <Tabs items={["Basic Example", "React Example"]}>
      <Tab value="Basic Example">
        ...
        const { data } = await authClient.profileImage.upload({// [!code highlight]
          base64Image: await fileToBase64(imageFile),// [!code highlight]
        });// [!code highlight]
...
We recommend to set up a trusted image origins, to prevent users from having any way of setting their profile picture to an image from an untrusted origin.
