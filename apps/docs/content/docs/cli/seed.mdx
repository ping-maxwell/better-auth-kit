...
export const seed = Seed({
    ...users({}, { 
        modelName: { // [!code highlight]
            user: "users", // [!code highlight]
            session: "sessions", // [!code highlight]
            account: "accounts", // [!code highlight]
        } // [!code highlight]
    }),
});
```
...
You can modify or append new fields to the `organizations`, `members`, `invitations`, and `teams` tables:
...
<Callout type="warn">
This is not yet implemented. However, you can still build each table manually.
</Callout>
